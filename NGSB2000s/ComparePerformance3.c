 ...
 * maxium likelihood method ... and now the approximate version of the latter (with AlphaBar chosen analytically).
 *
 */
...
main(int argc, char *argv[])
{
	...
	double		**simplex, **SmallSimplex, **sum;
	int		i, j, N, GN = 50, c, Trials, status = 1;
	...
	/* compute N */
	/* N = (int) ( Rho * Rho * Sigma1 * Sigma2 / (xi * xi * alpha * alpha) ); */
	/* if(N >= (int) 1e6 || N <= 1) {
		printf("N is unreasonable.  Maximum N is 1e6, you tried: %d\nGiving up...\n",N);
		return;
	} */
	...
	AFileName = (char *) malloc(100*sizeof(char));
        ...
        SmallSimplex = (double **) malloc(4*sizeof(double *));
        for(i=0; i<4; i++) SmallSimplex[i] = (double *)malloc(3*sizeof(double));
		...
		sum[2][0] = 0.0; 
	        ...
	        for(j=0; j<N; j++){
                ...
                status = SimplexMaxLikelihood(&LambdaMax, &SigmaBar1, &SigmaBar2, &AlphaBar, &XiBar, N, h1, h2,
                                              simplex, sum[2][0]/((double) N), sum[2][0]/((double) N), sum[1][1],
                                              7500, 7500, 1e-4, 1e-4);
                if(status == 0)
                        fprintf(MOutFile,"%e\t%% %e\t%e\t%e\t%e \n",  LambdaMax,SigmaBar1,SigmaBar2,AlphaBar,XiBar);
                else
                        fprintf(MOutFile,"%e\t%% %e\t%e\t%e\t%e %% WARNING !! DID NOT CONVERGE \n",  LambdaMax,SigmaBar1,SigmaBar2,AlphaBar,XiBar);
		fflush(NULL);

                /* approximate maximum likelihood statistic! */
                SmallSimplex[0][0] = 1.0;
                SmallSimplex[0][1] = 1.0;
                SmallSimplex[0][2] = xi;
                status = SimplexMaxLikelihood3(&SigmaBar1, &SigmaBar2, &XiBar, N, SmallSimplex, sum, 7500, 7500, 1e-4, 1e-4);
